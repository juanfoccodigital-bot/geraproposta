...
import { updateSiteSchema, parseBody } from "@/lib/validations";
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (error) return NextResponse.json({ error: "Erro ao atualizar site" }, { status: 500 });
    return NextResponse.json(data);
  } catch {
    return NextResponse.json({ error: "Erro interno do servidor" }, { status: 500 });
  }
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const rl3 = checkRateLimit(`sites:write:${user.id}`, WRITE_LIMIT);
    ...
    if (error) return NextResponse.json({ error: "Erro ao deletar site" }, { status: 500 });
