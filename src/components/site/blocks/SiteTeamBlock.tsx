...
import type { SiteTeamData } from "@/types/site";
...
export default function SiteTeamBlock({ data, themeColors }: Props) {
        ...
        {(data.members || []).map((member, i) => (
          <div key={i} className="flex flex-col items-center text-center">
            <div
              ...
              style={{ background: `${themeColors.gold}20`, border: `3px solid ${themeColors.gold}30` }}
            >
              {member.image ? (
                <img src={member.image} alt={member.name} className="w-full h-full object-cover" />
              ) : (
                <div className="w-full h-full flex items-center justify-center text-2xl font-bold" style={{ color: themeColors.gold }}>
                  {member.name?.charAt(0)?.toUpperCase()}
                </div>
              )}
            </div>
            <h3 className="font-semibold text-lg" style={{ color: themeColors.foreground }}>
              {member.name}
            </h3>
            <p className="text-sm opacity-60 mb-3" style={{ color: themeColors.foreground }}>
              {member.role}
            </p>
            {member.social && member.social.length > 0 && (
              <div className="flex gap-3">
                {member.social.map((s, j) => (
                  ...
                  >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill={themeColors.gold}>
